"""
高频行业轮动策略 V21 —— 绝对动量现金覆盖 + 周频调仓

基于V16（最佳综合表现，3/8 pass）

核心改动：
1. 新增绝对动量因子（20日个股自身收益，非交叉截面）
   - 牛市权重: 0.40*mom_z + 0.30*rel_rank + 0.30*abs_mom
   - 熊市权重: 0.20*mom_z + 0.40*rel_rank + 0.40*abs_mom
   效果：系统性下跌时所有ETF绝对动量为负 → 评分整体下移 → 更多ETF低于0分阈值 → 自然持有现金
   这不是熔断器，是评分系统的有机延伸
2. 周频调仓（每5个交易日），止损仍每日检查
   止损触发仅卖出，不补仓，等下次调仓日
3. 移除组合层面回撤熔断（已证明对反弹策略有害，被绝对动量自然替代）

可优化参数（≤5个）：K（持仓数量）: [3,4,5,6]
"""
...
def compute_scores_full(close: pd.DataFrame, symbols: list[str]) -> pd.DataFrame:
    ...
    abs_mom_raw = close[symbols].pct_change(ABS_MOM_LOOKBACK)
    abs_mom = abs_mom_raw.clip(-ABS_MOM_CLIP, ABS_MOM_CLIP) / ABS_MOM_CLIP
    ...
    w_mom = pd.Series(0.40, index=close.index)
    w_rel = pd.Series(0.30, index=close.index)
    w_abs = pd.Series(0.30, index=close.index)
    w_mom[bear_mask] = 0.20
    w_rel[bear_mask] = 0.40
    w_abs[bear_mask] = 0.40

    score = mom_z.mul(w_mom, axis=0) + rel_rank.mul(w_rel, axis=0) + abs_mom.mul(w_abs, axis=0)
...
def simulate(
    ...
    records = []
    stop_loss_events = 0
    days_since_rebalance = REBALANCE_INTERVAL  # force rebalance on first day
    ...
    for d in dates:
        ...
        should_rebalance = days_since_rebalance >= REBALANCE_INTERVAL
        ...
        if paused_until is not None and d <= paused_until:
            ...
            final_set = [s for s in current_weights[current_weights > 0].index]
...
def judge_report(close: pd.DataFrame, symbols: list[str], result: dict, version: str) -> dict:
    ...
    # cash utilization stats
    avg_holdings = np.mean([w.get("avg_holdings", 0) for w in windows]) if windows and "avg_holdings" in windows[0] else 0
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V21")

    print("\n=== JUDGE REPORT V21 ===")
    print(json.dumps(report, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v21")
