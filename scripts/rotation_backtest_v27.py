...
def compute_scores_full(close: pd.DataFrame, symbols: list[str]) -> pd.DataFrame:
    # Precompute z-scores for all needed lookbacks
    zscores = {}
    for L in [5, 10, 20]:
        ret = close[symbols].pct_change(L)
        zscores[L] = vectorized_zscore(ret)
    ...
    # Bull: ensemble(L=10, L=20), Bear: ensemble(L=5, L=10)
    bull_mom = (zscores[10] + zscores[20]) / 2
    bear_mom = (zscores[5] + zscores[10]) / 2

    mom_z = bull_mom.copy()
    bear_dates = bear_mask[bear_mask].index
    mom_z.loc[bear_dates] = bear_mom.loc[bear_dates]
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V27")

    print("\n=== JUDGE REPORT V27 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v27")
