...
TRAIN_OBJECTIVE_RECENT_DAYS = 120  # V8: 目标函数只看训练模拟序列最后120个交易日
...
def grid_search_train(
    ...
    for K in PARAM_GRID_K:
        ...
        if sim.empty:
            continue
        r = sim["net_return"].tail(TRAIN_OBJECTIVE_RECENT_DAYS)
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V8")

    print("\n=== JUDGE REPORT V8 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v8")
