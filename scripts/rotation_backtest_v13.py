...
# V11: 组合层面快速回撤熔断
PORTFOLIO_DD_THRESHOLD = -0.06  # 净值自阶段性高点回撤超过此值则空仓
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V13")

    print("\n=== JUDGE REPORT V13 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v13")
