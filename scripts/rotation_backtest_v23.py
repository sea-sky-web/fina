"""
高频行业轮动策略 V23 —— 逆波动率加权 + 移除组合熔断

基于V16（最佳综合，3/8 pass）。

V21失败：改评分权重毁alpha。V22失败：绝对收益门控是滞后信号，错过反弹。

V23思路：不改选股信号，只改仓位大小。
1. 逆波动率加权：每只ETF权重 ∝ 1/vol(20d)，低波动板块仓位更大
   - 直接降低组合波动率 → Sharpe分母下降
   - 趋势市中低波动的防御板块也能获得较好动量分
   - 暴跌市中高波动板块自动减仓
2. 移除组合层面回撤熔断（V3/V12/V13已证明有害，V16中虽有但可能未频繁触发）

其他全部与V16相同：评分函数、止损、滞后缓冲、K网格搜索等。
"""
...
def target_weights_invvol(final_set, symbols, vol_row):
    weights = pd.Series(0.0, index=symbols)
    if not final_set:
        return weights
    vols = vol_row.reindex(final_set).replace(0, np.nan).dropna()
    if vols.empty:
    ...
    raw_weights = raw_weights.clip(upper=MAX_SYMBOL_WEIGHT)
    total = raw_weights.sum()
    if total > 0:
        raw_weights = raw_weights / total
    weights.loc[raw_weights.index] = raw_weights
    for s in final_set:
        if s not in raw_weights.index:
            weights[s] = 1.0 / len(final_set)
    total = weights.sum()
    if total > 0:
...
def simulate(score_full, returns_full, close_full, symbols, dates, holdings, cost_rate_one_side=COST_RATE_ONE_SIDE):
    ...
    vol_full = close_full[symbols].pct_change().rolling(VOL_LOOKBACK).std()
    ...
    for d in dates:
        ...
        vol_row = vol_full.loc[d] if d in vol_full.index else pd.Series(dtype=float)
        target = target_weights_invvol(final_set, symbols, vol_row)
...
def main():
    ...
    report = judge_report(close, symbols, result, "V23")

    print("\n=== JUDGE REPORT V23 ===")
    print(json.dumps({k: v for k, v in report.items()}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v23")
