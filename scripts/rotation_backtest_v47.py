...
def simulate(
    ...
    # V47: Cross-sectional dispersion filter
    ...
    for d in dates:
        ...
        # V47: Bear market half-position scaling
        if bear_mask.loc[d]:
            target = target * BEAR_SCALE
        if low_disp_mask.loc[d]:
            target = target * DISP_SCALE
        if not bear_mask.loc[d] and not low_disp_mask.loc[d]:
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V47")

    print("\n=== JUDGE REPORT V47 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v47")
