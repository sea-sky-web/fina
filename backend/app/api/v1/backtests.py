...
@router.get("/rotation-core", response_model=BacktestResult)
def rotation_core_backtest(
    start: Annotated[date | None, Query()] = None,
    end: Annotated[date | None, Query()] = None,
    ...
    cost_bps: Annotated[float, Query(ge=0, le=100)] = 5.0,
    benchmark: Annotated[str, Query()] = "510300.SH",
    min_liquidity_score: Annotated[float, Query(ge=0, le=100)] = 0.0,
    min_risk_score: Annotated[float, Query(ge=0, le=100)] = 60.0,
    risk_managed: Annotated[bool, Query()] = True,
) -> BacktestResult:
    return run_rotation_core_backtest(
        start=start,
        end=end,
        top_n=top_n,
        rebalance=rebalance,
