...
相对 V2 唯一改动（V3 用组合层面仓位缩放控制回撤，但触发了自动回滚——胜率变差；
诊断认为组合层面缩放是滞后指标，会同时砍掉后续反弹的参与度）：

    改为个股层面止损：任何持仓浮亏（自建仓价起算的累计收益）跌破 STOP_LOSS_THRESHOLD
    （-8%，对应目标里的单窗口最大亏损上限）时，当天强制换出，并进入
    COOLDOWN_DAYS（5个交易日）冷却期——冷却期内即使分数转正也不能重新买入，
    避免刚止损就被同一信号立刻买回来回震荡。

    这样风险控制下沉到个股层面，不影响其他持仓正常参与上涨，理论上应该比V3的
    组合级仓位缩放更精准。

V2 的两个改动（缓冲区滞后换仓 + 固定双周期集成动量）保持不变。
"""
...
def simulate(
    ...
    for d in dates:
        ...
        if paused_until is not None and d <= paused_until:
            ...
            target.loc[list(stopped_out)] = 0.0
        else:
            row_score = score.loc[d].reindex(symbols)
            target = target_weights_hysteresis(
...
def main() -> None:
    ...
    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v4")
