...
def compute_scores_full(close: pd.DataFrame, symbols: list[str]) -> pd.DataFrame:
    ...
    for L in MOMENTUM_LOOKBACK_SET:
        ...
        vol = close[symbols].pct_change().rolling(L).std()
        vol = vol.replace(0, np.nan)
        risk_adj_ret = ret / vol
        mom_components.append(vectorized_zscore(risk_adj_ret.fillna(0)))
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V26")

    print("\n=== JUDGE REPORT V26 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v26")
