...
def simulate(
    ...
    # Graduated bear market scaling based on MA20/MA60 ratio
    bench_close = close_full[BENCHMARK]
    bench_ma20 = bench_close.rolling(20).mean()
    bench_ma60 = bench_close.rolling(60).mean()
    ma_ratio = (bench_ma20 / bench_ma60).reindex(dates).fillna(1.0)
    # ratio=1.0 → scale=1.0, ratio=0.95 → scale=0.5, ratio≤0.90 → scale=0.3
    bear_scale = (1.0 + (ma_ratio - 1.0) * 10).clip(0.3, 1.0)
    ...
    for d in dates:
        ...
        # V29: Graduated bear market position scaling
        scale = float(bear_scale.loc[d]) if d in bear_scale.index else 1.0
        if scale < 1.0:
            target = target * scale
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V29")

    print("\n=== JUDGE REPORT V29 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v29")
