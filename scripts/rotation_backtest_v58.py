...
def compute_scores_full(close: pd.DataFrame, symbols: list[str], amount: pd.DataFrame) -> pd.DataFrame:
    ...
    # V58: 量价齐升确认 —— A股短线交易核心信号（仅牛市启用，熊市中放量下跌会误判）
...
def simulate(
    ...
    for d in dates:
        ...
        # V58: Bear market half-position scaling
        if bear_mask.loc[d]:
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V58")

    print("\n=== JUDGE REPORT V58 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v58")
