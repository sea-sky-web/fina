...
# V11: 组合层面快速回撤熔断
PORTFOLIO_DD_THRESHOLD = -9.99  # 净值自阶段性高点回撤超过此值则空仓
...
def compute_scores_full(close: pd.DataFrame, symbols: list[str]) -> pd.DataFrame:
    ...
    w_mom = pd.Series(0.5, index=close.index)
    w_rel = pd.Series(0.5, index=close.index)
...
def simulate(
    ...
    for d in dates:
        ...
        if paused_until is not None and d <= paused_until:
            ...
            # V18: 所有持仓标的的20日动量均为负时，缩减到仅持有1只
            if len(final_set) > 1:
                mom20 = close_full[symbols].pct_change(20)
                if d in mom20.index:
                    held_mom = mom20.loc[d].reindex(final_set)
                    if (held_mom.dropna() < 0).all() and not held_mom.dropna().empty:
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V19")

    print("\n=== JUDGE REPORT V19 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v19")
