"""入口：PYTHONPATH=backend python -m app.rotation_engine 或 python scripts/rotation_backtest.py"""
...
def main_multi(multi_config: MultiAssetConfig | None = None) -> None:
    ...
    if basic_path.exists():
        ...
        name_map = dict(zip(basic["symbol"], basic["name"], strict=False))
...
def cli() -> None:
    """命令行分发：--multi 多资产模式，--validate 三层交叉验证，默认单类模式."""
    import sys
