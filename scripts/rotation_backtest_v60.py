...
def compute_scores_full(close: pd.DataFrame, symbols: list[str], amount: pd.DataFrame) -> pd.DataFrame:
    mom_components = []
    ...
    # V60: 量价齐升确认 —— A股短线交易核心信号（仅牛市启用，熊市中放量下跌会误判）
    ...
    volume_bonus[bear_mask] = 0.0  # 熊市关闭量价加成，避免误判
...
def simulate(
    ...
    for d in dates:
        ...
        # V60: Bear market half-position scaling
        if bear_mask.loc[d]:
...
def run_walk_forward(close: pd.DataFrame, symbols: list[str], amount: pd.DataFrame) -> dict:
    t0 = time.time()
    score_full = compute_scores_full(close, symbols, amount)
...
def main() -> None:
    ...
    result = run_walk_forward(close, symbols, amount)
    report = judge_report(close, symbols, result, "V60")

    print("\n=== JUDGE REPORT V60 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v60")
