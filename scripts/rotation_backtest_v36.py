...
HYSTERESIS_BUFFER = 3
PARAM_GRID_K = [2, 3, 4, 5]
PARAM_GRID_BEAR_SCALE = [0.40, 0.50, 0.60]
...
def simulate(
    ...
    bear_scale_override: float = 0.55,
) -> pd.DataFrame:
    ...
    bench_ma20 = bench_close.rolling(20).mean()
    bench_ma60 = bench_close.rolling(60).mean()
    bear_mask = (bench_ma20 < bench_ma60).reindex(dates).fillna(False)
    BEAR_SCALE = bear_scale_override
    ...
    for d in dates:
        ...
        # V36: Bear market half-position scaling
        if bear_mask.loc[d]:
...
def grid_search_train(
    ...
    best = (PARAM_GRID_K[0], -np.inf, PARAM_GRID_BEAR_SCALE[1])
    for K in PARAM_GRID_K:
        for bs in PARAM_GRID_BEAR_SCALE:
            sim = simulate(score_full, returns_full, close_full, symbols, train_dates, K, bear_scale_override=bs)
            if sim.empty:
                continue
            r = sim["net_return"]
...
def run_walk_forward(close: pd.DataFrame, symbols: list[str]) -> dict:
    ...
    for i, (train_dates, test_dates) in enumerate(windows):
        t1 = time.time()
        best_K, train_obj, best_bs = grid_search_train(score_full, returns_full, close, symbols, train_dates)
        test_sim = simulate(score_full, returns_full, close, symbols, test_dates, best_K, bear_scale_override=best_bs)
        ...
        print(
            ...
            f"K={best_K} bs={best_bs:.2f} ret={window_return:+.2%} bench={bench_window_return:+.2%} "
...
def cost_erosion_test(
    ...
    for cost in [0.0, COST_RATE_ONE_SIDE]:
        ...
        for w in windows_meta:
            ...
            sim = simulate(score_full, returns_full, close_full, symbols, test_dates, w["best_K"], cost_rate_one_side=cost, bear_scale_override=w.get("best_bear_scale", 0.55))
            if sim.empty:
                continue
            step_dates = test_dates[:STEP_TRADING_DAYS]
...
def judge_report(close: pd.DataFrame, symbols: list[str], result: dict, version: str) -> dict:
    ...
    all_dates = restrict_to_common_history(close, symbols)
    windows_for_cost = [
        {"test_start": pd.Timestamp(w["test_start"]), "test_end": pd.Timestamp(w["test_end"]), "best_K": w["best_K"], "best_bear_scale": w.get("best_bear_scale", 0.55)}
        for w in windows
    ]
...
def main() -> None:
    ...
    report = judge_report(close, symbols, result, "V36")

    print("\n=== JUDGE REPORT V36 ===")
    print(json.dumps({k: v for k, v in report.items() if k != "windows_detail"}, ensure_ascii=False, indent=2))

    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v36")
