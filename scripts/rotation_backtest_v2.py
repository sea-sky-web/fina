...
相对 V1 的两个改动（对应 V1 诊断报告的问题1/问题2）：
    1. 缓冲区滞后换仓（buffer hysteresis）：当前持仓只要仍在 Top(K+HYSTERESIS_BUFFER)
       名单内且分数为正就继续持有，只有跌出缓冲区或转负才替换，用于压降换手/降低成本侵蚀。
    2. 动量周期从"网格搜索单一 L"改为"L=10 与 L=20 双周期集成"（固定权重各 0.5，
       不再作为可调参数），相对强度回看窗口固定为 60 日。网格搜索空间从 16 组 (L×K)
       缩减到 4 组（仅 K），用于消除 V1 中 L 的参数漂移（CV_L=0.35 超阈值）。

其余机制（单只权重上限、单日换手上限、交易成本、熔断、Walk-Forward 窗口设置）
与 V1 完全一致，便于版本对照。
"""
...
def target_weights_hysteresis(
    score_row: pd.Series, current_weights: pd.Series, holdings: int, buffer: int = HYSTERESIS_BUFFER
) -> pd.Series:
...
def simulate(
    ...
    for d in dates:
        ...
        if paused_until is not None and d <= paused_until:
            ...
            target = target_weights_hysteresis(row_score, current_weights, holdings)
...
def grid_search_train(
    score_full: pd.DataFrame, returns_full: pd.DataFrame, symbols: list[str], train_dates: pd.DatetimeIndex
) -> tuple[int, float]:
    best = (PARAM_GRID_K[0], -np.inf)
    for K in PARAM_GRID_K:
        sim = simulate(score_full, returns_full, symbols, train_dates, K)
        if sim.empty:
            continue
        r = sim["net_return"]
...
def run_walk_forward(close: pd.DataFrame, symbols: list[str]) -> dict:
    ...
    for i, (train_dates, test_dates) in enumerate(windows):
        t1 = time.time()
        best_K, train_obj = grid_search_train(score_full, returns_full, symbols, train_dates)
        test_sim = simulate(score_full, returns_full, symbols, test_dates, best_K)
...
def cost_erosion_test(
    ...
    for cost in [0.0, 0.0005, 0.00075, 0.001]:
        ...
        for w in windows_meta:
            ...
            sim = simulate(score_full, returns_full, symbols, test_dates, w["best_K"], cost_rate_one_side=cost)
            if sim.empty:
                continue
            step_dates = test_dates[:STEP_TRADING_DAYS]
...
def main() -> None:
    ...
    cost_results = cost_erosion_test(result["score_full"], result["returns_full"], symbols, all_dates, windows_for_cost)
    ...
    output_dir = Path("/Users/ster/Desktop/fina/artifacts/backtest_v2")
