...
from .config import DEFAULT_CONFIG, TRADING_DAYS_PER_YEAR, ClassConfig, StrategyConfig
...
def simulate(
    ...
    if len(dates) == 0:
        empty = pd.DataFrame(
            columns=["date", "gross_return", "cost", "net_return", "turnover", "n_holdings"]
        ).set_index("date")
        empty.attrs["stop_loss_events"] = 0
        empty.attrs["reentry_events"] = 0
        return empty
